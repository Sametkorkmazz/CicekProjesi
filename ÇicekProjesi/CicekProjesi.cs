...
namespace ÇicekProjesi
{
    ...
    class NeuralNetwork // Yapay sinir ağı sınıfı
    {
        ...
        public void programiEgit() // Eğitim ve Ana metot
        {
            ...
            Cicek[] CicekListesi = cicekListesiniOlustur(); // Elemanları Çiçek sınıfından oluşan ,150 elemanlı ÇiçekListesi oluşturma
            Console.Clear();
            for (int tabloDeneyi = 0; tabloDeneyi < 3; tabloDeneyi++) // 3 tablo oluşturmak için en dış döngü
            {
                double[,] dogrulukDegerleri = new double[3, 3]; // Doğruluk yüzdelerinin tutulacağı 2 boyutlu dizi, satırlar epok değerleri sütünlar lambda değerleri için
                ...
                for (int Lambdaİndexi = 0; Lambdaİndexi < 3; Lambdaİndexi++) // LambdaDegerleri dizisini gezen index
                {
                    ...
                    {
                        agirliklariOlustur(random); // Yeni epek miktarina başlamadan önce ağırlıkları tekrardan random oluşturma

                        for (int EpokMiktari = 0; EpokMiktari < EpokMiktarlari[Epokİndexi]; EpokMiktari++)
                        {
                            // ÇiçekListesi dizisini gezen döngü
                            for (int ÇiçekListesiİndexi = 0; ÇiçekListesiİndexi < 150; ÇiçekListesiİndexi++)
                            {
                                NöronToplamDegeriHesapla(CicekListesi, ÇiçekListesiİndexi, LambdaDegerleri[Lambdaİndexi]); // Her Nöronun toplam degerini hesaplama

                                EnBuyükNöronuBul(CicekListesi, ÇiçekListesiİndexi, LambdaDegerleri[Lambdaİndexi]); // Değeri en büyük çıkan Nöronu bulup, bakılan Çiçekle karşılaştırma
                            }
                        }


                        for (int ÇiçekListesiİndexi = 0; ÇiçekListesiİndexi < 150; ÇiçekListesiİndexi++) // Epoklar tamamlandıktan sonra, ağırlıklar değiştirilmeden doğruluk değerini bulma
                        {
                            NöronToplamDegeriHesapla(CicekListesi, ÇiçekListesiİndexi, LambdaDegerleri[Lambdaİndexi]);

                            dogrulukDegerleri[Epokİndexi, Lambdaİndexi] += dogrulukKontrol(CicekListesi, ÇiçekListesiİndexi); // Bakılan çiçek doğruysa 1 değilse 0 döndüren metot
                        }
                ...
                sonuclariYaz(EpokMiktarlari, LambdaDegerleri, dogrulukDegerleri, tabloDeneyi); // Konsola sonuçları yazan metot
            }
        }


        static void agirliklariOlustur(Random random) // Her nöron için 4, toplam 12 (0,1) arasında ağırlık oluşturma (0 ve 1 hariç)
        {
            double sayi;
            for (int NöronlarListesiİndex = 0; NöronlarListesiİndex < 3; NöronlarListesiİndex++)
            {
                for (int Ağırlıkİndexi = 0; Ağırlıkİndexi < 4; Ağırlıkİndexi++)
                {
                    ...
                    nöronlar[NöronlarListesiİndex].agırlıklar[Ağırlıkİndexi] = Math.Round(sayi, 1); // Ağırlıklarınğın virgül sonrasını 1 basamağa yuvarlama
                }
            }
        }
        ...
        {
            string path = Path.Combine(Directory.GetCurrentDirectory()) + "\\iris.data"; /*Bu satirdaki path stringi iris.data dosyasının -
            ...
            string[] dosyaOkuma = File.ReadAllLines(path); // Elemanları iris.data dosyasının satırlarından oluşan 150 elemanlı string dizisi

            string[][] dataDizisi = new string[150][];
            for (int i = 0; i < 150; i++) // Çiçeklerin özelliklerini ve isimlerini her virgülde ayırarak yeni diziye atama
            {
            ...
            for (int ÇiçekListesiİndexi = 0; ÇiçekListesiİndexi < 150; ÇiçekListesiİndexi++) // Çiçek Listesine çiçek adları ve özelliklerini ekleme
            {
                for (int ÇiçekÖzellikİndexi = 0; ÇiçekÖzellikİndexi < 4; ÇiçekÖzellikİndexi++)
                {
                    // String olarak okunan özellikleri double sayılara çevirme ve ona bölme
        ...
        static void NöronToplamDegeriHesapla(Cicek[] cicekListesi, int index, double lamda) // Nöron ağırlıkları çiçek girdileriyle çarpıp toplayan metot
        {
            double toplam;
        ...
        static void EnBuyükNöronuBul(Cicek[] cicekListesi, int index, double lamda) // Değeri en büyük çıkan Nöronu bulup ,bakılan Çiçeğin Nöronuyla karşılaştıran metot
        {
            ...
            {
                case "Iris-setosa":
                    if (enBuyukNoron != 0) // Bakılan çiçeğin nöronuyla ,büyük çıkan nöron aynı olmazsa ağırlıkları değiştirir
                    {
                        küçükNöron = 0;
        ...
        static int dogrulukKontrol(Cicek[] cicekListesi, int index) /* Epok miktarı tamamlandıktan sonra doğruluk yüzdesini bulan metot.
            ...
            switch (cicekListesi[index].ad) // Çiçeğin ismine göre caselerden birine girme
            {
                case "Iris-setosa":
                    if (enBuyukNoron == 0) // En büyük çıkan Nöronla çiçeğin nöronunu karşılaştırır. Doğruysa 1 döndürür ,yanlış ise 0 döndürür.
                                           // Döndürdüğü 1 veya 0 değeri toplam doğru sayısına eklenir. 0 döndürürse doğru sayısı değişmez, 1 döndürürse doğru sayısı 1 artar.
                    {
                        return 1;
                    }
        ...
        static void sonuclariYaz(int[] deneyMiktarlari, double[] lamdaDegerleri, double[,] dogrulukDegerleri, int deney) // 3 Deney sonucunu birer tablo halinde konsola yazan metot.
        {
            ...
            Console.Write(string.Format("{0,28}λ {1,17}λ {2,17}λ{3,25}\n\n", lamdaDegerleri[1], lamdaDegerleri[0], lamdaDegerleri[2], "Ortalama Doğruluk"));
            for (int i = 0; i < 3; i++)
            {
